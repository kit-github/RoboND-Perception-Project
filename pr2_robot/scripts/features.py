...
def get_hist(data, nbins=32, bins_range=(0,256)):
    ...
    norm_features = hist_features / (np.sum(hist_features) + eps)
...
def compute_color_histograms(cloud, using_hsv=False, nbins=32):
    ...
    norm_features = get_hist(channel_vals, nbins=nbins)
...
def compute_normal_histograms(normal_cloud, nbins=32):
    ...
    # Replace normed_features with your feature vector
    normed_features = get_hist(norm_vals, nbins=nbins, bins_range=(-1,1))
